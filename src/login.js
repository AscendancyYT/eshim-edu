...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    await sendTelegramLog(`User logged in: ${user.email} at ${new Date().toISOString()}`);
    alert('Muvaffaqiyatli kirish!');
    // Redirect or handle successful login here
  } catch (error) {
    await sendTelegramLog(`Login failed for ${email}: ${error.message} at ${new Date().toISOString()}`);
...
signupForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const user = userCredential.user;
    await sendTelegramLog(`New user registered: ${name} (${user.email}) at ${new Date().toISOString()}`);
    alert('Muvaffaqiyatli ro‘yxatdan o‘tish!');
  } catch (error) {
    await sendTelegramLog(`Signup failed for ${email}: ${error.message} at ${new Date().toISOString()}`);
    alert(`Xatolik: ${error.message}`);
  }
});
...
toggleSwitch.addEventListener('change', () => {
  ...
  flipCard.style.transform = toggleSwitch.checked ? 'rotateY(180deg)' : 'rotateY(0deg)';
});