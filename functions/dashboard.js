...
async function advanceTurnTx(gameId, payload) {
  ...
  await runTransaction(gameRef, (gameData) => {
    ...
    const nextPlayer = gameData.players[0] === me ? gameData.players[1] : gameData.players[0];
...
function listenToGame(gameId) {
  ...
  const cb = (snapshot) => {
    ...
    if (!data) { enterLobby(); currentGameId = null; return; }
...
async function sendGameRequest(friendName) {
  ...
  if (snap.empty) { setText(requestStatus, "No user found with that name."); return; }
...
async function submitAnswer() {
  ...
  if (isNaN(val)) { setText(duelStatus, "Please enter a valid number!"); return; }
  ...
  if (data.currentPlayer !== currentPlayerName || data.status !== "active") return;
...
if (currentPlayerName) {
  ...
  onValue(myIncoming, (snapshot) => { renderIncomingRequests(snapshot); });
}

// --- UI events (duel) ---
sendRequestButton?.addEventListener("click", () => {
  const friendName = (friendNameInput?.value || "").trim();
  if (!friendName) { setText(requestStatus, "Please enter a friend's full name."); return; }
  if (!currentPlayerName) { setText(requestStatus, "Your profile name is missing."); return; }
  if (friendName === currentPlayerName) { setText(requestStatus, "You cannot invite yourself!"); return; }
  sendGameRequest(friendName).catch(() => setText(requestStatus, "Failed to send request. Try again."));
});
...
requestList?.addEventListener("click", (e) => {
  ...
  else if (target.classList.contains("declineRequest")) declineRequest(id).catch(console.error);
});
...
sidebarLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    const map = { grades: gradesSection, eshimpay: eshimpaySection, entertainment: entertainmentSection };

    [gradesSection, eshimpaySection, entertainmentSection].forEach((el) => {
      if (!el) return;
      if (el === map[section]) { el.classList.remove("hidden"); el.classList.add("active"); }
